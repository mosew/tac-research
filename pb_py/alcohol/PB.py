class PB(object):
    ...
    def __init__(self):
        ...
        self.u = np.concatenate((np.zeros(self.pad), self.u))[::2]
        self.y = np.concatenate((np.zeros(self.pad), self.y))[::2]
    ...
    def mcmc_init(self):
        ...
        # self.mats_temp = pbMatrices(self.thetas[:, 0], self.u, self.u, self.P, self.T, self.n, self.tau, self.k - 1)
        # self.y = np.array([self.mats_temp.compute_L_i(self.thetas[:, 0], self.u, i) for i in range(self.n)])
    ...
    def p_theta(self,theta):
        ...
        import numpy as np
        return truncnorm.pdf(theta[0], 0, np.Infinity, loc=0.0046, scale=0.0006) * truncnorm.pdf(theta[1],0, np.Infinity, loc=1.23, scale=0.12)
    ...
    def mcmc(self):
        ...
        while self.k < self.K:
            ...
            print self.alph * self.mats_prev.vhat
            ...
            if c > acc:
                ...
                if self.k > self.burnin:
                    print "Rejected!\n", c, acc
                    print "Theta:", self.mats_prev.theta, self.mats_try.theta
                    rejected += 1.
