...
%     plot(t_TAC_5122{i},data_TAC_5122{i},':')
    ...
    %                  (training, test)
    rosenEstBrACTAC = rosencode(1,i);
    plot(rosenEstBrACTAC(:,1)*60,rosenEstBrACTAC(:,2),':')

    xlim([0 500])
    ylim([0,75])
    
    if i==5
        h=legend('trained on ep 6','trained on eps 6:7','trained on eps 6:8','trained on eps 6:9','sampled BrAC','interpolated BrAC','Est. BrAC, prev. method, trained on episode 1');
        pos=get(h,'position');
